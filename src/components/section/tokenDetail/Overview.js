...
function Overview({ tokenId }) {
  ...
  useEffectOnce(() => {
    async function getTokenOverviewData() {
      if (tokenId) {
        const res = await getTokenOverview(tokenId);
        setOverview(res);
      }
    }
  ...
  //   let dateArr = [];
  //   overview.map((row) => {
  //     const temp = { value: row?.price, time: dateToTimeStamp(row?.timestamp) };
  //     dateArr.push(temp);
  //   });

  //   areaSeries.setData(dateArr);
  // }, [overview]);
  return (
    <div className="mt-5">
      <div
